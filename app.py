...
def parse_scopus_data(df):
    ...
    def clean_and_split_references(ref_string):
        if not ref_string or pd.isna(ref_string):
            return set()
        
        # Split references using the unique "); " pattern
        raw_elements = ref_string.split('); ')
        cleaned_refs = []
        ...
        for item in raw_elements:
            ...
            if not item:
                continue
            # If the item doesn't end with a closing parenthesis (due to the split), append it back
            if not item.endswith(')'):
                item += ')'
            cleaned_refs.append(item)
            
        return set(cleaned_refs)
    
    # Process the text using the updated "); " boundary delimiter logic
    df['Ref_Set'] = df['References'].apply(clean_and_split_references)
    return df
...
if uploaded_file is not None:
    ...
    if st.session_state.is_clustered and selected_cluster != "All" and len(display_graph.nodes) > 0:
        ...
        total_cluster_count = len(display_graph.nodes)
